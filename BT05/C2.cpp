...
void loang(char mine_map[][col_max], bool bool_map[][col_max], int x, int y);
...
int main()
{
    ...
    while(!quit) {
        ...
        {
            count_turn++;
            if(mine_map[x - 1][y - 1] == '0') loang(mine_map, bool_map, x, y);
            else bool_map[x - 1][y - 1] = true;
...
void loang(char mine_map[][col_max], bool bool_map[][col_max], int x, int y)
{
    ...
    for (int p = x - 2; p <= x; p++)
    {
        ...
        {
            if (q < 0) q = 0;
            if (mine_map[p][q] != '0') bool_map[p][q] = true;
            else if (!bool_map[p][q] && mine_map[p][q] == '0') loang(mine_map, bool_map, p + 1, q +1);
        }
    }
}
void print_map(char mine_map[][col_max], bool bool_map[][col_max], int m, int n)
